...
const AlbumSong = () => {
  const [albumPageSong, setAlbumPageSong] = useState([]);
  let { song } = useParams();
  ...
  useEffect(() => {
    const albumSong = async () => {
      ...
      setAlbumPageSong(result?.data);
      console.log(result);
    };
    albumSong();
  }, []);

  if (albumPageSong.length > 0) {
    return (
      <>
        <section
              ...
              {albumPageSong?.map((product) => (
                ...
                >
                  ...
                  <div
                    className={
                      isToggleOff
                        ? " text-white bg-zinc-800 my-5 flex items-center justify-center"
